...
# 1. 데이터 준비 및 탐색
data = pd.read_csv("D:\\Git\\bigdata\\5. Geographic\\공공보건의료기관현황.csv", index_col=0, encoding='CP949', engine='python')
# print(data.head()) #작업내용 확인용 출력

## 주소에서 시도, 군구 정보 분리
addr = pd.DataFrame(data['주소'].apply(lambda v: v.split()[:2]).tolist(),columns=('시도', '군구'))
# print(addr.head())  #작업내용 확인용 출력
...
# 4) 행정구역 인구수 컬럼 추가하기
population = pd.read_excel("D:\\Git\\bigdata\\5. Geographic\\행정구역_시군구_별__성별_인구수_2.xlsx")
...
# (1) 블록맵 데이터 파일 열기
data_draw_korea = pd.read_csv(path+"\\5. Geographic\\data_draw_korea.csv", index_col=0, encoding='UTF-8', engine='python')
...
def draw_blockMap(blockedMap, targetData, title, color ):
    ...
    plt.tight_layout()
    plt.savefig('.\\5. Geographic\\' + 'blockMap_' + targetData + '.png')
    plt.show()      
